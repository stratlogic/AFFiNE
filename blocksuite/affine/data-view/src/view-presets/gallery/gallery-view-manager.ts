...
class GalleryProperty extends PropertyBase {
  override get hide$() {
    return computed(() => {
      const data = this.view.data$.value as GalleryViewData | undefined;
      return !!data?.columns?.find(c => c.id === this.id)?.hide;
    });
  }
  override hideSet(hide: boolean) {
    (this.view as GallerySingleView).dataUpdate((data: GalleryViewData) => {
      ...
      col.hide = hide;
      return { columns: cols } as Partial<GalleryViewData>;
    });
  }
  override move(_position: InsertToPosition) {
    // Standard move not supported yet
  }
}
...
export class GallerySingleView extends SingleViewBase<GalleryViewData> {
  ...
  override isShow(_rowId: string): boolean {
    return true; // Simple filter for the grid
  }

  override propertyGetOrCreate(propertyId: string): Property {
    return new GalleryProperty(this, propertyId);
  }
}
